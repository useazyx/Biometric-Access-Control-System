 ...
 * - v1.5.0 (2026-09-09): responsible_employee_cpf deixou de ser opcional no tipo,
 *                        porque a rota e o serviço sempre exigiram ele
 */
...
interface CreateVisitorBody {
  ...
  company?: string
  visit_reason?: string
  // Obrigatório: todo visitante precisa de um funcionário que responda por ele
      ...
      switch (error.message) {
        ...
        case "Funcionário responsável não encontrado":
          return reply.status(404).send({
            error: error.message,
            solution: "Verifique se o CPF está correto e se o funcionário está cadastrado",
          })
        case "Esta pessoa já está cadastrada como visitante":
          return reply.status(409).send({ error: error.message })
        default:
          // Se for um erro desconhecido, manda resposta genérica
          return reply.status(400).send({ error: "Erro ao cadastrar visitante" })
      }
    }
  }
}
