...
export class UpdateStudentService {
  ...
  private prepareUpdateData(request: UpdateStudentRequest) {
    ...
    if (request.course !== undefined) {
      // Permite string vazia para limpar o campo
      updateData.course = request.course || null;
    }
    if (request.className !== undefined) {
      // Permite string vazia para limpar o campo
      updateData.class = request.className || null;
    }
    if (request.status) updateData.status = request.status;
    if (request.responsible !== undefined) {
      // Permite string vazia para limpar o campo
      updateData.responsible = request.responsible || null;
    }
