...
import type { Period, StudentStatus } from "@prisma/client"
...
export class CreateStudentService {
  async execute(data: CreateStudentRequest): Promise<StudentResult> {
    return await prisma.$transaction(async (tx) => {
      ...
      await this.checkExistingStudent(person.id, tx)
      await this.checkExistingRm(data.rm, tx)

      // Se passou nas validações, cria o cadastro
      return this.createStudent(data, person.id, tx)
    })
  }
  ...
  private async checkExistingStudent(personId: number, tx: any) {
  ...
  private async checkExistingRm(rm: string, tx: any) {
    // RM tem que ser único, então busca se já existe algum com esse número
    const existingRm = await tx.student.findUnique({ where: { rm } })
  ...
  private async createStudent(data: CreateStudentRequest, personId: number, tx: any): Promise<StudentResult> {
    // Cria o estudante com status fornecido ou "active" por padrão
    const student = await tx.student.create({
      data: {
        rm: data.rm,
        period: data.period,
        course: data.course || null,
        class: data.className || null,
