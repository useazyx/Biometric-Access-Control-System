...
export class UpdateEmployeeService {
  ...
  private async prepareUpdateData(roleName?: string, active?: boolean, admissionDate?: string) {
    // Começa vazio - vai juntando o que precisa atualizar
    const updateData: { role_id?: number; active?: boolean; admission_date?: Date } = {};
    ...
    // Se mandaram atualizar a data de admissão, converte e coloca nos dados
    if (admissionDate) {
      updateData.admission_date = new Date(admissionDate);
    }
  ...
  private async updateEmployee(
    employeeId: number,
    updateData: { role_id?: number; active?: boolean; admission_date?: Date }
  ) {
