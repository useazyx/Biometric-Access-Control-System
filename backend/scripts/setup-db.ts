/**
 * setup-db.ts - Deixa o banco pronto sozinho, antes da API subir
 * # Pra que serve?
 * - Criar o banco no PostgreSQL se ele ainda não existir
 * - Aplicar as migrações pendentes
 * - Rodar o seed (que é idempotente) pra garantir unidade, cargos e o usuário administrador
 * - Assim o `npm run dev` basta: ninguém precisa lembrar de db:deploy e db:seed na mão
 * Feito por: Arthur Roberto Weege Pontes
 * Versão: 1.0.0
 * Data: 2026-09-10
 * Alterações:
 * - v1.0.0 (2026-09-10): Primeira versão, criada pra simplificar a subida do projeto
 *
 * Como rodar:
 *   npm run db:setup
...
// Separa a URL do banco em duas: a do banco do projeto e a do banco "postgres",
...
// Aplica as migrações que ainda não foram pro banco
...
// Roda o seed. Ele usa upsert, então pode rodar toda vez sem duplicar nada —
// e é isso que garante que a senha do admin seja sempre a do .env.
function runSeed() {
  ...
  if (result.status !== 0) {
    console.error("\n❌ O seed falhou. A API não vai ter usuário pra logar.\n")
    process.exit(1)
  }
}
...
function main() {
  if (!DATABASE_URL) {
    console.error("\n❌ Falta o DATABASE_URL no backend/.env.\n")
    console.error("   Copie o modelo:  cp .env.example .env\n")
    process.exit(1)
  }

  const { databaseName, serverUrl } = splitDatabaseUrl(DATABASE_URL)
